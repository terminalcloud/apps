...
select_sid(){
  cd /root
  wget https://raw.githubusercontent.com/terminalcloud/apps/master/others/lb_stacks.json
  clear
  ...
  read option
  case $option in
    1) sed -i "s/sid/$UBUNTU/g" lb_stacks.json ;;
    2) sed -i "s/sid/$CENTOS/g" lb_stacks.json ;;
    3) sed -i "s/sid/$PHP/g" lb_stacks.json ;;
    4) sed -i "s/sid/$NODEJS/g" lb_stacks.json ;;
    5) sed -i "s/sid/$RUBY/g" lb_stacks.json ;;
    6) sed -i "s/sid/$DJANGO/g" lb_stacks.json ;;
    0) custom_sid ;;
    *) echo "Invalid option, assuming 1, Ubuntu basic Image"; sed -i "s/sid/$UBUNTU/g" lb_stacks.json ;;
  esac
}
...
select_number(){
  ...
  read kind
  case $kind in
    1) sed -i "s/cpuq/100/g" lb_stacks.json && sed -i "s/ramq/1600/g" lb_stacks.json ;;
    2) sed -i "s/cpuq/200/g" lb_stacks.json && sed -i "s/ramq/3200/g" lb_stacks.json ;;
    3) sed -i "s/cpuq/400/g" lb_stacks.json && sed -i "s/ramq/6400/g" lb_stacks.json ;;
    *) echo "Invalid option, assuming Small"; sed -i "s/cpuq/100/g" lb_stacks.json && sed -i "s/ramq/1600/g" lb_stacks.json ;;
  esac
}
...
get_tokens(){
  ...
  sed -i "s/utoken/$utoken/g" lb_stacks.json
  sed -i "s/atoken/$atoken/g" lb_stacks.json
  sed -i "s/sid/$sid/g" lb_stacks.json
  sed -i "s/IP/$IP/g" lb_stacks.json
}

create_nodes(){
    for ((i=1;i<=$num;i++));
    do
      curl -L -X POST -H 'Content-Type: application/json' -d @lb_stacks.json api.terminal.com/v0.1/start_snapshot
...
select_db(){
  ...
  read option
  case $option in
    1) sed -i "s/sid/$MYSQL/g" lb_stacks.json ;;
    2) sed -i "s/sid/$MYSQLC/g" lb_stacks.json ;;
    3) sed -i "s/sid/$MONGODB/g" lb_stacks.json ;;
    *) echo "Invalid option, assuming 1, MySQL on Ubuntu"; sed -i "s/sid/$MYSQL/g" lb_stacks.json ;;
  esac
}
